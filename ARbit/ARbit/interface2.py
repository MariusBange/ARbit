...
from PyQt5.QtWidgets import QWidget, QApplication, QLabel, QVBoxLayout, QHBoxLayout, QGridLayout, QGroupBox
import sys
import cv2
...
from pyQtOpenGL import PyQtOpenGL
...
class SatelliteChecklist(QtWidgets.QDialog):
    ...
    def stateChanged(self, index):
        ...
        if checked and self.satellites[index.row()] not in self.selectedSatellites and len(self.selectedSatellites) < 3:
            ...
            self.selectedSatellites.append(self.satellites[index.row()])
        elif not checked and self.satellites[index.row()] in self.selectedSatellites:
            for satellite in self.selectedSatellites:
                if satellite == self.satellites[index.row()]:
                    self.selectedSatellites.remove(satellite)
                    break

        self.app.openGLScene.selectedSatellites = self.selectedSatellites
...
class ControlWidget(QGroupBox):
    ...
    def __init__(self, satelliteSelection, modelLibrary, app):
        ...
        self.sliderScale.setMinimum(25)
        self.sliderScale.setMaximum(200)
        self.sliderScale.setValue(110)
        self.sliderScale.setFixedSize(120, 30)
        self.sliderScale.valueChanged.connect(self.scaleChanged)
        self.labelScaleValue = QtWidgets.QLabel(f"   {self.sliderScale.value()/100}")
        self.buttonReset = QtWidgets.QPushButton(text="Reset scale")
        self.buttonReset.clicked.connect(self.resetScale)
        ...
        self.grid.addWidget(self.buttonReset, 4, 0, 1, 4)

        self.setLayout(self.grid)

    def scaleChanged(self):
        self.scale = self.sliderScale.value() / 100
        self.labelScaleValue.setText(f"   {self.scale}")
        print(self.scale)
        self.app.openGLScene.scale = self.scale
...
class App(QWidget):
    ...
    def __init__(self, processor: Processor, satelliteSelection, modelLibrary):
        ...
        self.openGLScene = PyQtOpenGL(processor)

        self.webcamHeight = 480
        webcamResolutionWidth = self.openGLScene.webcam.get(cv2.CAP_PROP_FRAME_WIDTH)
        webcamResolutionHeight = self.openGLScene.webcam.get(cv2.CAP_PROP_FRAME_HEIGHT)
        ratio = self.webcamHeight / webcamResolutionHeight
        self.webcamWidth = int(webcamResolutionWidth * ratio)
        self.openGLScene.setFixedSize(self.webcamWidth, self.webcamHeight)
        ...
        hbox.addWidget(self.openGLScene)
        self.setLayout(hbox)
    ...
    #             cvImg = satellite.render(cvImg, tvecs, camToGlobeRotation, 17.5, 20, (255, 255, 0), rvecs)
